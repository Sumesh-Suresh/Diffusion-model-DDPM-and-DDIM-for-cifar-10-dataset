...
class DiffusionModel(nn.Module):
    def __init__(
        ...
        alphas = 1. - self.betas
        # cumulative products for current and previous timesteps.
       
        self.alphas_cumprod = torch.cumprod(alphas,dim=0)
        # self.alphas_cumprod_prev = torch.cat(torch.tensor([1.0],device= self.device),self.alphas_cumprod[:-1])
        self.alphas_cumprod_prev =  torch.cat([torch.tensor([1.0], device=self.device), self.alphas_cumprod[:-1]])
        
        # Pre-computing values needed for forward process.
    ...
    def model_predictions(self, x_t, t):
        ##################################################################
        # A noised image x_t is given and we predict x_0 and the additive
        # noise to predict the additive noise, use the denoising model.
        # Using extract function from utils.py. See
        # get_posterior_parameters() for usage examples.
        ##################################################################
        
        pred_noise = self.model(x_t,t)
        # print("shape of pred_noise = ",pred_noise.shape)
        # print("shape of x = ",x_t.shape)
        alpha_t = extract(self.alphas_cumprod,t,x_t.shape)
        x_0 = 1/torch.sqrt(alpha_t)*(x_t-torch.sqrt(1-alpha_t)*pred_noise)
    ...
    def predict_denoised_at_prev_timestep(self, x, t: int):
        ##################################################################
        # Given x at timestep t, predicting the denoised image at
        # x_{t-1}, and return the predicted starting image.
        # noise to predict the additive noise, use the denoising model.
        ##################################################################
        _,x_0= self.model_predictions(x,t)
        # mu, var, log_var = self.get_posterior_parameters(x_0,x,t)
        posterior_mean_coef1 = extract(self.posterior_mean_coef1,t,x.shape)
        posterior_mean_coef2 = extract(self.posterior_mean_coef2,t ,x.shape)
        posterior_variance = extract(self.posterior_variance,t,x.shape)
        mu_t = posterior_mean_coef1*x_0 + posterior_mean_coef2*x
        var_t = posterior_variance.to(self.device)

        # if t > 0: z = torch.normal(0,1,size=x.shape)
        # else: z= 0
        z = torch.normal(0,1,size=x.shape).to(self.device)

        # print("shape of x = ", x.shape)
        # print("shape of variance = ",var_t.shape)

        pred_img =  mu_t + torch.sqrt(var_t)*z
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
        return pred_img, x_0
    ...
    def ddim_step(self, batch, device, tau_i, tau_isub1, img, model_predictions, alphas_cumprod, eta):
        ##################################################################
        # Computing the output image for a single step of the DDIM
        # sampling process.
        ##################################################################
        # Step 1: Predict x_0 and the additive noise for tau_i
        
        # z = torch.normal(0,1,size= img.shape).to(device)
        # alpha_ti = extract(alphas_cumprod,tau_i,img.shape)
        # x_0 = 1/torch.sqrt(alpha_ti)*(img-torch.sqrt(1-alpha_ti)*z)
        # et = self.model(img,tau_i)
        ...
        pred,x_0 = model_predictions(img,tau_i)
        
        if torch.all(tau_i > 0 ):
            if torch.all(tau_isub1>0):
                z = torch.normal(0,1,size=img.shape,device =self.device)
            else:
                # z = torch.zeros_like(img,device = self.device)
                return img,x_0
        # Step 2: Extract \alpha_{\tau_{i - 1}} and \alpha_{\tau_{i}}
        alpha_ti = extract(alphas_cumprod,tau_i,img.shape)

       
        alpha_ti_prev = extract(alphas_cumprod,tau_isub1,img.shape) 
        
        beta = extract(self.betas,tau_isub1,img.shape)

        # Step 3: Compute \sigma_{\tau_{i}}
        var_ti = eta*(1-alpha_ti_prev)/(1-alpha_ti)*beta
       

        # Step 4: Compute the coefficient of \epsilon_{\tau_{i}}
        coeffecient_e = torch.sqrt(1-alpha_ti_prev-var_ti)
        

        # Step 5: Sample from q(x_{\tau_{i - 1}} | x_{\tau_t}, x_0)
        # Using the reparameterization trick

        mu_ti = torch.sqrt(alpha_ti_prev)*x_0 + coeffecient_e*pred
        # import pdb
        # pdb.set_trace()
        img = mu_ti  + torch.sqrt(var_ti)*z
